...
class Step1(StepTemplateCd):
    ...
    def check_command(self, line):
        ...
        if line in self.allowed_commands:
            ...
            hint = ("{{gb:Well done! Just look at one more item.}}")

        else:
            if self.first_time:
                hint = ("{{rb:Use}} {{yb:cat}} {{rb:to look at two of the " +\
                         "objects around you.}}")
            else:
                hint = ("{{rb:Use the command}} {{yb:%s}} {{rb:to progress.}}")\
    ...
    def check_command(self, line):
        ...
        if self.get_fake_path() == '~/my-house/kitchen' or self.get_fake_path() == '~/my-house':
            ...
            if line == "cd .." or line == 'cd ../':
                hint = ("{{gb:Good work! Now replay the last command using " +\
            ...
            if self.num_turns_in_home_dir == 0:
                hint = ("{{gb:Cool! Now use}} {{yb:cd town}} {{gb:to head to town.}}")
